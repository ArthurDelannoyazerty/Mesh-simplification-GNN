...
import cProfile
# Assuming Triangle, Vector, Mesh, and file reading functions are defined elsewhere

def simplification2(file_path, simplification_rate):
    ...
    part_file_extension = os.path.splitext(file_path)[1].lower()  # Ensure lowercase
    ...
    print(f"Triangles created: {len(triangles)}")

    original_num_faces = len(faces)
    mesh = Mesh(triangles)
    print("Starting mesh simplification")
    copy_file('a.txt', 'benchmark\data\output\csv\metrics_results.csv')

    simplified_triangles = mesh.simplify(simplification_rate, original_num_faces)

    simplified_vertices, simplified_faces = get_vertices_faces_from_triangles(simplified_triangles)
    difference = original_num_faces - len(simplified_faces)
    

    print(f"Simplified mesh: Vertices {len(simplified_vertices)}, Faces {len(simplified_faces)}")

    #write_data_out(simplified_vertices, simplified_faces, f'{simplification_rate:.2f}', basename)
    write_stl_out(simplified_vertices, simplified_faces, 'benchmark\data\output\stl'+ basename, f'{simplification_rate:.2f}')

    return simplified_vertices, simplified_faces, difference


# Example usage
# vertices, faces, difference = simplification2('path_to_your_file.stl', 0.5)
